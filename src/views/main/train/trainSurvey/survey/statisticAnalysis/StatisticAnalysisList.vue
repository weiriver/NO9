    ...
    <TablePage :totalCount="totalCount">
      ...
      <div slot="query">
        ...
        <Form :inline="true" class="form-inline" label-width="140px" :model="searchForm" size="mini"
              ref="searchForm">
          <FormItem label="问卷单号">
            <Input v-model="searchForm.surveyId"/>
          </FormItem>
          <FormItem label="所属区域">
      ...
      <div slot="TableSlot">
        <Table stripe :data="tableData" :height="tableHeight" border style="width: 100%" v-if="tableHeight">
          ...
          <TableColumn label="问卷单号" prop="surveyId"></TableColumn>
          <TableColumn label="是否提交问卷">
            <template slot-scope="scope">
              <span v-if="scope.row.readFlag">是</span>
              <span v-else>否</span>
            </template>
          </TableColumn>
          <TableColumn label="站点编号" prop="stationCode"></TableColumn>
          <TableColumn label="发起时间" prop="commitTime"></TableColumn>
          <TableColumn
             label="操作"
             width="140">
            <template slot-scope="scope">
              <Button @click="toDetail(scope.row.id)" type="text" size="small">查看</Button>
            </template>
          </TableColumn>
        </Table>
      </div>
...
export default {
  ...
  methods: {
    ...
    toDetail(id) {
      this.$router.push(`/trainSurvey/statisticAnalysisListDetail/${id}`)
    },
    async init() {
