    ...
    <TablePage :totalCount="totalCount">
      ...
      <div slot="TableSlot">
        <Table stripe :data="tableData" :height="tableHeight" border style="width: 100%" v-if="tableHeight">
          ...
          <TableColumn label="业务标题" prop="name"></TableColumn>
          <TableColumn label="业务类型" prop="type"></TableColumn>
          <TableColumn label="申请时间">
            <template slot-scope="scope">
              {{ scope.row.applyTime | getTxt }}
            </template>
          </TableColumn>
          <TableColumn label="截止时间">
            <template slot-scope="scope">
              {{ scope.row.deadlineTime | getTxt }}
            </template>
          </TableColumn>
          ...
          <TableColumn label="当前步骤">
            <template slot-scope="scope">
              {{ scope.row.option | getTxt }}
            </template>
          </TableColumn>
