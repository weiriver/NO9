...
<template>
  <div>
    ...
    <div style="width: 500px" class="mb25">
    ...
    <Form :inline="true" :rules="rules" class="form-inline" label-width="100px" :model="Obj" size="mini"
      ...
      <FormItem label="出入仓库" prop="fromWarehouseTo">
        <WarehouseFromTo ref="WarehouseFromTo" v-model="Obj.fromWarehouseTo"></WarehouseFromTo>
      </FormItem>
    ...
    <div>
      ...
      <div class="mt25 mb20 tc">
        <Button type="primary" @click="submit">调拨</Button>
      </div>
    </div>
...
import WarehouseFromTo from './WarehouseFromTo'
...
export default {
  ...
  data: function () {
    return {
      ...
      rules: {
        ...
        fromWarehouseTo: [
          // {required: true, type: 'array', message: '请输入出入仓库', trigger: 'change'},
  ...
  methods: {
    ...
    chooseGoodsName(item) {
      ...
      console.log(this.tableData[this.activeIndex].amountMinLabel == null)
    ...
    async submit() {
      ...
      if (formDataCheckedOk && tableDataCheckedOk) {
        this.Obj.fromWarehouse = this.Obj.fromWarehouseTo[0]
        this.Obj.toWarehouse = this.Obj.fromWarehouseTo[1]
        this.Obj.dataList = this.tableData
        const res = await this.postJson('ws', {apiCode: 501802, content: this.Obj})
        if (res.code == 0) {
          this.showMsg('调拨开单成功')
          this.tableData = []
          this.Obj = {}
    ...
    getFlowInfo(flowId) {
      this.postJsonWithMask('common', {
        ...
        const userId = JSON.parse(sessionStorage.getItem('loginInfo')).uid
