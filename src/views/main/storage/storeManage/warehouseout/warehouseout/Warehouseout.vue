    ...
    <TablePage :totalCount="totalCount">
      ...
      <div slot="btn_groups">
        <Select class="search-select" size="mini" v-model="state" @change="stateChange">
          <Option label="待出库" value="todo"></Option>
          <Option label="已出库" value="done"></Option>
        </Select>
      </div>
      ...
      <div slot="query">
        ...
        <Form :inline="true" class="form-inline" label-width="140px" :model="searchForm" size="mini"
          ...
          <FormItem label="出库时间" prop="name" v-if="state === 'done'">
      ...
      <div slot="TableSlot">
        <Table stripe :data="tableData" :height="tableHeight" border style="width: 100%" v-if="tableHeight">
          ...
          <TableColumn label="出库人员" prop="createByName"></TableColumn>
          <TableColumn label="备注" prop="remark"></TableColumn>
          <TableColumn
            label="操作"
            width="140">
            <template slot-scope="scope">
              <Button v-if="state !== 'todo'" @click="editItem(scope.row.id)" type="text" size="small">
                  详情
              </Button>
              <Button v-if="state === 'todo' && Auth(501401)" @click="wareOut(scope.row.id)" type="text" size="small">
                出库
              </Button>
...
<script>
  export default {
    ...
    methods: {
      ...
      editItem (id) {
        this.$router.push(`/store/warehouseoutDetail/${id}?state=${this.state}` )
      },
      wareOut (id) {
        this.$router.push(`/store/warehouseoutBilling/${id}?state=${this.state}` )
      },
      async init() {
