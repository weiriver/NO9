...
<template>
  <div>
    ...
    <Form :inline="true" class="form-inline" label-width="100px" :model="Obj" size="mini"
          ref="Obj">
    ...
    <div>
      <Table stripe :data="tableData" border style="width: 100%" class="add-minus">
        ...
        <TableColumn label="设备编号">
          <template slot-scope="scope">
            <Input :disabled="scope.row.goodsType != 'device'" v-model="scope.row.goodsCode"
                   :class="{'_is-error':scope.row.goodsCodeRequired}" @focus="goodsCodeClicked(scope)"></Input>
        ...
        <TableColumn label="数量(大)">
          <template slot-scope="scope" width="100">
            <div class="inline-block width60">
              <Input v-model="scope.row.amountMax" width="60"
        ...
        <TableColumn label="备注">
          <template slot-scope="scope" width="60">
            <Input v-model="scope.row.remark" width="60"
                   :class="{'_is-error':scope.row.amountMaxRequired}"></Input>
          </template>
        </TableColumn>
      </Table>
      ...
      <div class="mt25 mb20 tc">
        <Button type="primary" @click="submit" :disabled="flag">损耗申请</Button>
      </div>
    </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      activeIndex: 0, // 设备编号点击的时候的 index
      selectedCode: [],
      dversionId: '',
      goodsId: '',
  ...
  methods: {
    async getWareHouseId() {
      ...
      if (res.code == 0) {
        ...
        this.fromWarehouse = res.content.warehouseId
        this.flag = false
      } else {
    ...
    selectChange(selection, row) {
      var codeIndex = this.selectedCode.findIndex((item) => {
        return item === this.tableData[this.activeIndex].goodsCode
      })
      if (codeIndex > -1) {
    ...
    chooseGoodsCode(item) {
      this.$set(this.tableData[this.activeIndex], 'goodsCode', item.code)
    ...
    goodsCodeClicked(item) {
      console.log('设备编号--被点击获取编号item', item, item.$index)
      this.activeIndex = item.$index
      if (this.tableData[this.activeIndex].goodsType == 'device') {
        this.goodsCodeDialogVisible = true
        this.dversionId = item.row.dversionId
        this.goodsId = item.row.goodsId
        this.getGoodsCodeTableData(1)
        this.Event.$emit('pageInit', 1)
        console.log('this.dversionId', this.dversionId)
    ...
    chooseGoodsName(item) {
      ...
      this.tableData.splice(this.activeItemIndex, 1)
      this.tableData.splice(this.activeItemIndex, 0, ...arr)
      // this.$set(this.tableData[this.activeItemIndex], 'getNOFlag', true)
      // this.$set(this.tableData[this.activeItemIndex], 'no', '11111')
    ...
    // 获取物品列表信息
    async getGoodsList() {
      const res = await this.postJson('ws', {apiCode: 501006})
      if (res.code == 0) {
        this.allGoodsList = res.content
        this.goodsTableData = res.content.device
      }
    },
    // 检查增减行的数据格式是否正确storeAdd
    ...
    async submit() {
      ...
      if (formDataCheckedOk && tableDataCheckedOk) {
        let resultArr = []
        resultArr = this.tableData.map(v => {
          return {
            id: v.goodsId,
            type: v.goodsType,
        ...
        this.Obj.resultList = resultArr
        this.Obj.nameX = this.Obj.title
        const res = await this.postJson('biz', {apiCode: 200079, content: this.Obj})
        if (res.code == 0) {
          this.showMsg('损耗申请成功')
          this.$router.push('/store/consumption')
        }
      }
    },
    // tab点击事件
