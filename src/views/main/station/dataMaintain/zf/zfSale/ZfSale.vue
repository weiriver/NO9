    ...
    <TablePage :totalCount="totalCount">
      ...
      <div slot="query">
        ...
        <Form :inline="true" class="form-inline" label-width="140px" :model="searchForm" size="mini"
          ...
          <FormItem label="大厅编号" prop="hallCode">
            <Input v-model="searchForm.hallCode"></Input>
          </FormItem>
          <FormItem label="大厅名称" prop="hallName">
      ...
      <div slot="TableSlot">
        <Table stripe :data="tableData" :height="tableHeight" border style="width: 100%" v-if="tableHeight">
          ...
          <TableColumn label="大厅名称" prop="hallName"></TableColumn>
          <TableColumn label="大厅编号" prop="hallCode"></TableColumn>
          <TableColumn label="游戏类型" prop="gameTypeName"></TableColumn>
          <TableColumn label="销售时间" prop="saleDate" min-width="100"></TableColumn>
          <TableColumn label="销售量" prop="sales"></TableColumn>
          <TableColumn
             label="操作"
             width="170">
            <template slot-scope="scope">
              <Button @click="$router.push('/dataMaintain/zfSaleAdd/' + scope.row.id)" type="text" typeOptions>修改
              </Button>
              <Button @click="$router.push('/dataMaintain/zfSaleDetail/' + scope.row.id)" type="text" typeOptions>查看
              </Button>
...
export default {
  ...
  data: function () {
    return {
      recordObj: {apiCode: 200105, sysModule: 'station', tableName: 'zf_sale_data'},
      downloadObj: {sysModule: 'station', hostBizObj: 'templet_zfzx', hostBizId: 'templete_xlsx'},
      searchForm: {
        page: 1
      },
      deleteId: '', // 删除的id
