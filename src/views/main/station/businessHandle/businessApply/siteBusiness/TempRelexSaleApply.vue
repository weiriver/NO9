  ...
  data() {
    ...
    return {
      ...
      ruleInline: {
        ...
        newAccountLimitTmp: [
          {required: true, message: '临时销售限额不能为空', trigger: 'blur'},
          {validator: isMPNumber, message: '临时销售限额只能输入数字', trigger: 'blur'}
        ]
      }, // 校验规则
      Obj: {flow: {content: ''}},
      // 站点编号集合
  ...
  methods: {
    ...
    init() {
      ...
      if (this.$route.query.from == 'apply') {
        ...
        this.draft = false
        const buz = JSON.parse(sessionStorage.getItem('tempRelexSaleApply'))
      ...
      if (!this.Obj.stationCode) {
        ...
        this.postJson('station', {
          ...
          if (res.code == 0) {
            ...
            // 站点基本信息
            this.$set(this.Obj, 'oldAccountLimitTmp', response.stationHotline ? response.stationHotline.accountLimitTmp : '')
    ...
    getDraftDetail(flowId, bizId, instanceId) {
      this.postJsonWithMask('common', {
        ...
        this.Obj.newAccountLimitTmp = this.Obj.newAccountLimitTmp ? this.Obj.newAccountLimitTmp + '' : ''
