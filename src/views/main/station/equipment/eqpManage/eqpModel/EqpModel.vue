    ...
    <TablePage :totalCount="totalCount">
      ...
      <div slot="query">
        ...
        <Form :inline="true" class="form-inline" label-width="140px" size="mini" :model="dataForm" ref="dataForm">
          <FormItem label="设备型号名称" prop="modelName">
            <Input v-model="dataForm.modelName"></Input>
          </FormItem>
      ...
      <div slot="TableSlot">
        <Table stripe :data="tableDataFacilities" :height="tableHeight" border style="width: 100%" v-if="tableHeight">
          ...
          <TableColumn label="型号名称" prop="deviceModelName"></TableColumn>
          <TableColumn label="是否内置" prop="isEmbeded">
            <template slot-scope="scope">
              <span>{{scope.row.isEmbeded === false ? '否': '是'}}</span>
            </template>
          </TableColumn>
          <TableColumn label="设备类型" prop="typeName"></TableColumn>
          <TableColumn label="厂家信息" prop="factoryInfo"></TableColumn>
...
export default {
  ...
  methods: {
    ...
    toAdd() {
      this.$router.push(`/equipment/eqpModelAdd`)
    },
    toDetail(id) {
      this.$router.push(`/equipment/eqpModelDetail/` + id)
    },
    editItem(id) {
      this.$router.push(`/equipment/eqpModelAdd/` + id)
    },
    opendelete(id) {
