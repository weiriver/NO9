    ...
    <TabPane label="业务详情">
      <Form :model="Obj" :rules="ruleInline" ref="Obj" label-width="200px" class="Obj ruleForm">
        ...
        <FormItem label="原销售限额">
          <Input v-model="Obj.oldAccountLimitTmp" :disabled="!editableObj.oldAccountLimitTmp.editable" :maxlength='10'/>
        </FormItem>
        <FormItem label="投注站地址">
          <Input v-model="Obj.address" :disabled="!editableObj.address.editable" :maxlength='50'/>
        </FormItem>
        <FormItem label="申请临时销售限额" prop="newAccountLimitTmp">
          <Input v-model="Obj.newAccountLimitTmp" :disabled="!editableObj.newAccountLimitTmp.editable" :maxlength='7'/>
        </FormItem>
        <!-- 处理和附件 -->
        <h3>
          处理意见
...
export default {
  ...
  data() {
    const isMPNumber = (rule, value, callback, source, options) => {
      ...
      if (value == '-') {
        this.Obj.newAccountLimitTmp = 0
        callback()
      } else if (this.Regexp.isMPNumber(value * 1)) {
        this.Obj.newAccountLimitTmp = -Math.abs(value)
        console.log(this.Obj.newAccountLimitTmp)
        callback()
      } else {
    ...
    return {
      instanceId: JSON.parse(sessionStorage.getItem('todosTempRelexSaleApply')) ? JSON.parse(sessionStorage.getItem('todosTempRelexSaleApply')).instanceId : '',
      spinShow: false,
      currentNodeCode: '',
      uploadConf: {
        sysModule: 'flow',
      ...
      if (!this.Obj.stationCode) {
        ...
        this.postJson('biz', {
          ...
          if (res.code == 0) {
            ...
            this.Obj.oldAccountLimitTmp = response.accountLimitTmp
  ...
  mounted() {
    const buz = JSON.parse(sessionStorage.getItem('todosTempRelexSaleApply'))
    console.log(buz)
